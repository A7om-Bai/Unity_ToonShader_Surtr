    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(varryings i) : SV_Target
            {
                ...
                float3 specular = 0.3 * light.color * pow(saturate(dot(N, H)), 64) * _SpecularStrength * NoL; // 简单的高光计算
                ...
                float3 stepRim = smoothstep(0.35, 0.55, rimLight) * 0.2;

                #ifdef _USE_RAMP_SHADOW
                    half3 finalcolor = baseColor.rgb * rampColor * (isShadowArea ? 1 : 1.2) + specular + stepRim; // 设置最终颜色为基础贴图颜色乘以渐变贴图颜色，在非阴影区域稍微提亮
                #else
                    half3 finalcolor = baseColor.rgb * halflambert + specular + stepRim;
                #endif
